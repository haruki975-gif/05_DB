...
AS
SELECT 
	E.EMP_ID AS 사번, E.EMP_NAME AS 이름, 
...
-- V_EMP에서 대리 직급만 조회
SELECT 사번, 이름, 직급명 FROM V_EMP
WHERE 직급명 = '대리';

/* VIEW를 이용한 DML 사용 시 문제점 */

-- DEPARTMENT 테이블 복사한 DEPT_COPY2 테이블 생성
...
CREATE OR REPLACE VIEW V_DCOPY2 -- LOCATION_IN 제외
AS SELECT DEPT_ID, DEPT_TITLE FROM DEPT_COPY2; -- TITLE, ID만 보임

-- V_DCOPY2에 다시 INSERT
INSERT INTO V_DCOPY2 VALUES('A1', '마케팅1팀');
-- NULL을 ("KH17_LEB"."DEPT_COPY2"."LOCATION_ID") 안에 삽입할 수 없습니다
-- V_DCOPY2를 이용하면 LOCATION_ID 컬럼에 INSERT할 수 없기 때문에 자동으로 NULL 추가
--> LOCATION_ID는 NOT NULL 제약조건이 설정되어 있기 때문에 NULL 삽입 불가!!

--> 테이블, VIEW 구조를 파악하지 못하고 있으면 
--> VIEW를 이용한 DML 수행 시 잦은 오류 발생

/* VIEW READ ONLY OPTION
 * -> VIEW는 ALTER 불가 -> CREATE OR REPLACE 사용
 */
CREATE OR REPLACE VIEW V_DCOPY2 -- LOCATION_IN 제외
AS SELECT DEPT_ID, DEPT_TITLE FROM DEPT_COPY2
...
-- 정의 되어 있지 않습니다
-- == NEXTVAL가 한 번도 호출된적이 없다

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; -- 100
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL; -- 100 (오류없음)

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; -- 105, 110, 115, 120 -> 5씩 증가 확인

-- TB_TEST 테이블에 INSERT 시 PK 컬럼값을 시퀀스로 얻어와서 삽입
...
UPDATE TB_TEST SET 
TEST_NO = SEQ_TEST_NO.NEXTVAL 
WHERE TEST_NAME='짱구'; -- 160, 170 ~ 200
...
/* WORKBOOK 계정으로 접속 */
SELECT COUNT(*) FROM TB_IDX_TEST; -- 100만개 행
-- TEST_NO(PK, INDEX), TEST_ID(NOT NULL)

-- 테이블 형태 확인
SELECT * FROM TB_IDX_TEST WHERE TEST_NO = 1;
-- 1 TEST1

/* 인덱스를 이용하지 않고 조회하기
 * -TEST_ID 컬럼값이 'TEST500000'인 행 조회
 */
SELECT * FROM TB_IDX_TEST WHERE TEST_ID='TEST500000'; -- 0.107

/* 인덱스를 이용해서 조회하기
 * -TEST_ID 컬럼값이 500000인 행 조회
 */
SELECT * FROM TB_IDX_TEST WHERE TEST_NO=500000; -- 0.021

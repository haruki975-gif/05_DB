...
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D9'; -- 선동일, 송종기, 노옹철 3행
...
SELECT 
	...
	E.SALARY, E.DEPT_CODE, E.HIRE_DATE
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
WHERE SALARY = (
	SELECT MIN(SALARY)
...
SELECT 
	EMP_ID, EMP_NAME, D.DEPT_TITLE, J.JOB_NAME, SALARY 
FROM EMPLOYEE E
 ...
 * 그룹 함수 또는 GROUP BY에 사용된 컬럼명만 작성 가능
 */
...
SELECT 
	NVL(DEPT_TITLE, '없음') DEPT_TITLE, COUNT(*)
FROM EMPLOYEE 
LEFT JOIN DEPARTMENT ON (DEPT_ID=DEPT_CODE)
GROUP BY DEPT_TITLE
HAVING 
	COUNT(*) = (
		SELECT MIN(COUNT(*)) FROM EMPLOYEE -- 서브쿼리 결과 : 2
		GROUP BY DEPT_CODE
	);
...
SELECT EMP_NO, JOB_CODE, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY IN (
	SELECT MAX(SALARY) -- 7행
	FROM EMPLOYEE 
	GROUP BY DEPT_CODE
);
...
JOIN JOB J ON (J.JOB_CODE=E.JOB_CODE)
WHERE 
	E.EMP_ID NOT IN ( -- 사수 역할을 갖지 않는 사원만 조회 (17행)
		SELECT DISTINCT MANAGER_ID 
		FROM EMPLOYEE 
		WHERE MANAGER_ID IS NOT NULL
	);
...
-- 2) 1번 SELECT문을 인라인뷰로 사용하여 상위 3개의 부서만 조회
SELECT
...
-- OVER() 괄호 내에 정렬 기준 작성(ORDER BY ASC, DESC)
--> 정렬되면서 순위가 지정됨
...
WHERE
-- 1) ROWNUM 이용 -> 1부터 시작하지 않아서 조회 결과 X
-- ROWNUM BETWEEN 2 AND 7;
-- 2) 순위 이용 -> 실존 컬럼 이용 시 문제 없음!
	"순위" BETWEEN 3 AND 7;
...
-- 1. 전지연 사원이 속해있는 부서원들을 조회하시오 (단, 전지연은 제외) -D1
-- 사번, 사원명, 전화번호, 고용일, 부서명
SELECT 
	EMP_ID, EMP_NAME, PHONE, HIRE_DATE, D.DEPT_TITLE
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE=D.DEPT_ID)
WHERE DEPT_CODE = (
	SELECT DEPT_CODE
...
-- 2. 고용일이 2010년도 이후인 사원들 중 
-- 급여가 가장 높은 사원의 사번, 사원명, 전화번호, 급여, 직급명을 조회하시오.
SELECT 
...
-- 4. 2010년도에 입사한 사원과 -- 유재식 D6 J3
-- 부서와 직급이 같은 사원을 조회하시오
-- 사번, 이름, 부서코드, 직급코드, 고용일
SELECT
	EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE 
...
-- 5. 87년생 여자 사원과 동일한 부서이면서  --> 전지연, D1, 214
...
SELECT 
	E.EMP_ID , E.EMP_NAME, 
	NVL(D.DEPT_TITLE,'소속없음') AS DEPT_TITLE, J.JOB_NAME, E.HIRE_DATE
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE=D.DEPT_ID)
JOIN JOB J ON (E.JOB_CODE=J.JOB_CODE)
WHERE HIRE_DATE = (
...
SELECT 
	...
	FLOOR(
		MONTHS_BETWEEN(
			SYSDATE, TO_DATE(19 || SUBSTR(EMP_NO, 1, 6), 'YYYYMMDD')
		) / 12
	) AS "만 나이",
	TO_CHAR(

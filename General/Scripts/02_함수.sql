...
--모든 사원의 급여 평균 조회
SELECT AVG(SALARY) 
FROM EMPLOYEE; --4091140.86956521739130434782608695652174
...
--> GROUP BY 또는 서브쿼리 이용
------------------------------------------------------------------
/* MAX(컬럼명), MIN(컬럼명) : 최대/최소값 */

--부서코드가 'D6'인 사원 중
--제일 높은 급여, 제일 낮은 급여 조회
SELECT MAX(SALARY), MIN(SALARY) 
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D6'; -- 4900000, 3800000

--모든 사원 중 이름순으로 오름차순 정렬했을 때
...
--모든 사원 중 입사일이 가장 빠른 사원, 늦은 사원 조회
...
--EMPLOYEE 테이블에서
--부서코드가 'D5', 'D6'인 사원의 수를 반환
SELECT COUNT(*)
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5', 'D6'); -- 9행
...
/* 방법 2 - COUNT(컬럼명) 이용 */
-- PHONE 컬럼값이 NULL인 경우 제외하고 카운트
SELECT COUNT(PHONE) FROM EMPLOYEE; -- 20행

--EMPLOYEE 테이블에 존재하는 부서코드 수 조회
